...
<script id='FriendCopyRow' type='text/html'>
	<tr>
		<td>${isbn}</td><td>${title}</td>
		<td class="authors">${authorsText}</td>
		<td class="starts">
			<form type="score_friend" idCopy='${id}' rate='${rate}' >
		...
		<td><img src="http://graph.facebook.com/${facebookId}/picture"></td>
	</tr>
</script>
<script type="text/javascript">
	var MyFriendsLibraryPage = new function() {
		this.init = function() {
			$("#myFriendsLibrary").tablesorter();
			_UpdateMyFriendsLibrary();
			$("#refreshMyFriendsLibrary").click(function() {
				_UpdateMyFriendsLibrary();
			})
		}
		
		function _UpdateMyFriendsLibrary() {
			$.getJSON("resources/copy/user/friends", function(p_copys) {
				$("#myFriendsLibrary tbody").html($("#FriendCopyRow").tmpl(p_copys));
				$("#myFriendsLibrary").trigger("update");
				
				$("form[type='score_friend']").children().not(":radio").hide();
				$("form[type='score_friend']").stars({
					 disabled: true
				});
				$("form[type='score_friend']").each(function() {
	...
	$(document).ready(function() {
		MyFriendsLibraryPage.init();
	});
</script>
</head>
<body>
My friends library
	<table id="myFriendsLibrary" class="tablesorter">
		<thead>
			<tr>
				<th>ISBN</th>
				<th>Title</th>
	...
	<button id='refreshMyFriendsLibrary'
